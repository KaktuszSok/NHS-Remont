...
{
    public static class FractureExtensions
    {
        ...
        public static Bounds TransformBoundVertices(this Transform from, Transform to, Bounds bounds)
        {
        ...
        public static Bounds GetCompositeMeshBounds(this GameObject go, bool includeInactive = false, bool isSharedMesh = false)
        {
            var bounds = go.GetComponentsInChildren<MeshFilter>(includeInactive)
                .Select(mf =>
                {
                    ...
                    var localBound = mf.transform.TransformBoundVertices(go.transform, mesh.bounds);
                    return localBound;
                })
