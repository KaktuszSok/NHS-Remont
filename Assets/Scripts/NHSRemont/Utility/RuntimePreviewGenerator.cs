...
//https://forum.unity.com/threads/runtime-preview-thumbnail-generator-open-source.500194/
// Minor changes by Kaktusz
namespace NHSRemont.Utility
{
	public class RuntimePreviewGenerator
	{
		...
		public const int PREVIEW_LAYER = 22;
		public static readonly Vector3 PREVIEW_POSITION = new Vector3(-9245f, 9899f, -9356f);

		private Camera renderCamera;
		private CameraSetup cameraSetup = new CameraSetup();

		private List<Renderer> renderersList = new List<Renderer>(64);
		private List<int> layersList = new List<int>(64);
		...
		private ProjectionPlane projectionPlaneHorizontal, projectionPlaneVertical;

#if DEBUG_BOUNDS
	private List<Transform> boundsDebugCubes = new List<Transform>( 8 );
	private Material boundsMaterial;
#endif

		private Camera m_internalCamera = null;
		...
		private Camera m_previewRenderCamera;
		...
		private bool m_mipmapDebugMode;
		public bool MipmapDebugMode
		{
			get { return m_mipmapDebugMode; }
			set { m_mipmapDebugMode = value;  }
		}

		private bool m_markTextureNonReadable;
		...
		public RuntimePreviewGenerator()
		{
		...
		public Texture2D GenerateMaterialPreview(Material material, PrimitiveType previewObject, int width = 64, int height = 64)
		{
			return GenerateMaterialPreviewWithShader(material, previewObject, null, null, width, height);
		}

		public Texture2D GenerateMaterialPreviewWithShader(Material material, PrimitiveType previewPrimitive, Shader shader, string replacementTag, int width = 64, int height = 64)
		{
		...
		public Texture2D GenerateModelPreview(Transform model, int width = 64, int height = 64, bool shouldCloneModel = false, bool useObjectRotation = false)
		{
			return GenerateModelPreviewWithShader(model, null, null, width, height, shouldCloneModel, useObjectRotation);
		}

		private static int counterForStoppingOverlappingModels = 0;
		public Texture2D GenerateModelPreviewWithShader(Transform model, Shader shader, string replacementTag, int width = 64, int height = 64, bool shouldCloneModel = false, bool useObjectRotation = false)
		{
			if (!model)
				return null;
			...
			try
			{
				...
				if (!isStatic)
				{
					Vector3 pos = PREVIEW_POSITION;
					pos.x += counterForStoppingOverlappingModels * 50;
					counterForStoppingOverlappingModels++;
					counterForStoppingOverlappingModels %= 400;
				...
				Vector3 boundsSize = 2 * boundsExtents;
			...
			boundsDebugCubes.Clear();
				...
				if (m_orthographicMode)
				{
					...
					renderCamera.orthographicSize = (1f + m_padding * 2f) * (maxY - minY) * 0.5f;
					renderCamera.aspect = aspect;
				}
				else
				{
				...
				result = new Texture2D(width, height, m_backgroundColor.a < 1f ? TextureFormat.RGBA32 : TextureFormat.RGB24, m_generateMipMaps, false);
				result.ReadPixels(new Rect(0, 0, width, height), 0, 0, true);
				//TextureUtils.StripChunks_sRGB_gAMA(result);
				if (m_generateMipMaps) //fix mipmap darkness
				{
					if(m_mipmapDebugMode) 
						TextureUtils.SetMipsToDebugMode(result);
					else
						TextureUtils.FixMipmapsBrightness(result);
				}
		...
		private void GetLayerRecursively(Transform obj)
		{
		...
		private void SetLayerRecursively(Transform obj, ref int index)
		{
...
#if DEBUG_BOUNDS
	private void CreateDebugCube( Vector3 position, Vector3 rotation, Vector3 scale )
	{
